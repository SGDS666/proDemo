...
// import { createPortal } from 'react-dom'; 不生效
...
export function Sortable({
  ...
  const handleRemove = removable
    ? (id: UniqueIdentifier) => onItemsChange(items.filter((item) => item !== id))
    : undefined;
  ...
  const onInputFinished = () => {
    ...
    if (!newValue) {
      setState({ createVisible: false, newValue: '' });
      return;
    }
    if (items.includes(newValue)) {
      message.error('该词语已存在');
      setState({ createVisible: false, newValue: '' });
      return;
    }
      ...
      onDragEnd={({ over }) => {
        ...
        if (over) {
          ...
          if (activeIndex !== overIndex) {
            onItemsChange(reorderItems(items, activeIndex, overIndex));
          }
        }
      }}
    ...
    >
      <Wrapper style={style} center>
        <SortableContext items={items} strategy={strategy}>
          <Container style={{ padding: 0, minWidth: 420 }}>
            ...
            {createVisible ? (
              <Input
                ...
                onChange={(e) => setState({ newValue: e.target.value })}
                onBlur={onInputFinished}
                onPressEnter={onInputFinished}
              />
            ) : (
              <Button type="dashed" onClick={() => setState({ createVisible: true })} size="small">
                <PlusOutlined /> 新增
              </Button>
            )}
          </Container>
