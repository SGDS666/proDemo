...
import { apiTagsAdd, apiTagsDirs } from '@/services/contacts';
...
const TagsCreate: React.FC<CreateFormProps> = (props) => {
  const { visible, onCancel, actionReload, parentId } = props;
  ...
  const { run: addRun, loading: addLoading } = useRequest(apiTagsAdd, {
    manual: true,
    onSuccess: () => {
  ...
  useEffect(() => {
    if (visible) {
      ...
      setState({ name: '', color, parent: parentId });
      form.resetFields();
      form.setFieldsValue({ parent: parentId ? parentId : '0', color });
    }
