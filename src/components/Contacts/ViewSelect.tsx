...
import { Input, Popover, Divider, theme } from 'antd';
import { useSetState } from 'ahooks';
import { apiViewsList } from '@/services/contacts';
...
const ViewSelect: React.FC<Props> = (props) => {
  const { onSelectAction, showCreate, children } = props;
    ...
    return views.map((item: any) => {
      ...
      return (
        <div key={viewId} onClick={() => onViewClick(viewId, name)}>
          <div className={styles['view-item']} style={{ color: token.colorText }}>{name}</div>
        </div>
      );
    });
  };
