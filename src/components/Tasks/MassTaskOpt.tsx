...
const MassTaskOpt: React.FC<CreateFormProps> = (props) => {
  const { visible, onCancel, actionReload, taskId, action } = props;
  ...
  const { run: countRun, loading: countLoading } = useRequest(apiTaskSendCount, {
    manual: true,
    onSuccess: (data) => {
      const { count, sendCount, customers } = data;
      setState({ choiceNum: count, skipNum: 0, limitNum: count, sendCount, customers });
    },
  });
  ...
  const onSelectFinished = async () => {
    ...
    const { match, exclude } = values;
    if (match && match.length) {
      await countRun({ match, exclude, mode });
    } else {
      setState({ choiceNum: 0 });
    }
  };
  ...
  const { run: taskAudit } = useRequest(apiMassTaskAudit, {
    manual: true,
    onSuccess: () => {
      onCancel();
  ...
  const handleSubmit = async (values: any) => {
    ...
    const { sendAtOnce, timeStart, mode, skipNum, limitNum } = state;
    const others = getOtherValues();
    if (action === 'change') {
      taskAudit({ ...values, ...others, gtid: taskId, mode });
    } else {
      taskRun({ ...values, ...others, sendAtOnce, timeStart, mode, skipNum, limitNum });
    }
  };
  ...
  // 获得已保存任务的其他设置
  const getTaskOtherValues = (data: any) => {
    const { track, timeSleep, replyTo, fromName, proxy, attNormal } = data;
    if (track) {
      setState({ track: { status: true } });
    }
    if (timeSleep) {
      setState({ timeSleep: { status: true, value: timeSleep } });
    }
    if (replyTo) {
      setState({ replyTo: { status: true, value: replyTo } });
    }
    if (fromName) {
      setState({ fromName: { status: true, value: fromName } });
    }
    if (proxy) {
      setState({ proxy: { status: true } });
    }
    if (attNormal) {
      setState({ attNormal: { status: true } });
    }
  };

  const { run: taskInfoRun } = useRequest(apiMassTaskInfo, {
    manual: true,
    onSuccess: (data: any) => {
      if (!data) return;
      const { match, exclude, gtid, total_count } = data;
      let { mode } = data;
      if (!mode) {
        mode = 'view';
      }
      if (action !== 'change') {
        countRun({ match, exclude, mode });
        getTaskOtherValues(data);
      }
      form.setFieldsValue(data);
      setState({ gtid, choiceNum: total_count, mode });
    },
  });
  ...
  const renderForm = () => {
    return (
      ...
      >
        {action === 'change' ? null : (
          <Form.Item hidden={renderHidden(0)}>
            {state.limitSetting ? null : (
              <div>
                <a
                  onClick={() => setState({ limitSetting: true })}
                  style={{ borderBottom: '1px solid blue', fontSize: 16 }}
                >
                  数量自定义
                </a>
              </div>
                ...
                <Space size="small">
                  <span>
                    ...
                    <InputNumber
                      ...
                      max={state.choiceNum}
                      value={state.skipNum}
                      onChange={(v: any) => setState({ skipNum: v })}
                      style={{ width: 120 }}
                    />
                  </span>
                  <span style={{ marginLeft: 24 }}>
                    ...
                    <InputNumber
                      ...
                      onChange={(v: any) => setState({ limitNum: v })}
                      style={{ width: 120 }}
                    />
                  </span>
                </Space>
        ...
        >
          <Radio.Group disabled={action === 'change'}>
  ...
  const renderTitle = () => {
    if (action === 'edit') {
      return (
        <span>
          编辑群发任务 <span style={{ fontSize: 12 }}>已选中人数:</span>{' '}
          <a>{countLoading ? <LoadingOutlined /> : state.choiceNum}</a>
        </span>
      );
    }
    if (action === 'change') {
      return (
        <span>
          修改群发任务 <span style={{ fontSize: 12 }}>已选中人数:</span>{' '}
          <a>{countLoading ? <LoadingOutlined /> : state.choiceNum}</a>
        </span>
      );
    }
    return (
      <span>
        新建群发任务 <span style={{ fontSize: 12 }}>已选中人数:</span>{' '}
        <a>{countLoading ? <LoadingOutlined /> : state.choiceNum}</a>
      </span>
    );
  };
  ...
  return (
    ...
    >
      ...
      {state.sendCount < state.choiceNum && action !== 'change' ? (
        <Alert
          message="群发额度不足"
          showIcon
