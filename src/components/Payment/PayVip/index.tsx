...
const PayVip: React.FC<OperationModalProps> = (props) => {
    ...
    onSuccess: (data) => {
      if (data) {
        ...
        if (goodsList && goodsList.length) {
          const { id, price, tips } = goodsList[1];
  ...
  const myFooter = () => {
    return (
      <div>
        ...
        <Button onClick={() => setState({ vipInfoVisible: true })} danger>
          会员特权
        </Button>
        <Button
          type="primary"
          onClick={handleSubmit}
          disabled={initialState?.currentUser?.uid !== initialState?.currentUser?.userid}
        >
          确认支付
        </Button>
      </div>
  ...
  const goodsButtons = (item: any, idx: number) => {
    ...
    if (state.goodsId === id) {
      return (
        ...
        >
          <div >
            {worth ? <div className="bestText">超值</div> : null}
            <div
              style={{
                color: '#03081a',
                fontSize: '20px',
            ...
            <div style={{ color: '#b57d1d' }}>
              {numeral(price / 100 / count).format('0,0.00')} 元/天
            </div>
          </div>
        </Radio.Button>
      );
  ...
  const onPayBalanceChange = (yuan: any) => {
    const { price, balance, goodsCount } = state;
  ...
  const getModalContent = () => {
    return (
      <>
        {state.tabKey === 'svip' ? (
          <div style={style1}>
            购买 <span style={{ color: '#e9b966', fontWeight: 700 }}>SVIP</span> 套餐:
          </div>
        ) : (
          <div style={style1}>
            购买 <span style={{ color: '#ff7b64', fontWeight: 700 }}>VIP</span> 套餐:
          </div>
        )}
        <div>
          ...
          >
            <Radio.Button
              ...
              style={{
                ...
                marginRight: 20,
                background: state.payApp === 'wechat' ? token.colorBgLayout : token.colorBgContainer,
              }}
            >
              <div style={{ marginTop: 24 }}>
                <Row>
                  <Col span={11}>
                    <div style={{ fontSize: 24, fontWeight: 700, paddingBottom: 12, color: token.colorText }}>
            ...
            <Radio.Button
              ...
              style={{
                height: 160,
                width: 360,
                background: state.payApp === 'alipay' ? token.colorBgLayout : token.colorBgContainer,
              }}
            >
              <div style={{ marginTop: 24 }}>
                <Row>
  ...
  return (
    ...
    >
      ...
      <VipInfo
        visible={state.vipInfoVisible}
        onCancel={() => setState({ vipInfoVisible: false })}
      />
    </Modal>
  );
};
