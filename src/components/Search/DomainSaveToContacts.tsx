...
const DomainSaveToContatcs: React.FC<FormProps> = (props) => {
  ...
  const { run: saveRun, loading: saveLoading } = useRequest(apiDomainSaveContacts, {
    ...
    onSuccess: (data: any) => {
      ...
      message.success(
        `精准邮箱保存数：${personalSave}，扣点：${personalCost}; 普通邮箱保存数：${genericSave}，扣点：${genericCost}`,
      );
      onCancel();
      actionReload();
    },
    ...
    >
      ...
      >
        <Form.Item
          ...
          name="types"
          rules={[{ required: true, message: '请选择保存邮箱类型' }]}
        >
