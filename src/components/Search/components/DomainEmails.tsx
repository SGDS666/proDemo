...
import { apiDomainEmails, apiEmailSave } from '@/services/search';
import { useRequest } from '@umijs/max';
import { Col, message, Row, Spin, Table, Button } from 'antd';
...
const CompanyEmails: React.FC<InfoProps> = (props, ref) => {
  ...
  const { run, loading, refresh } = useRequest(apiDomainEmails, {
  ...
  const renderAction = (record: any) => {
    ...
    if (hidden) {
      return (
        <Button
          onClick={() => saveRun({ id, type: 'generic' })}
          type="link"
          size="small"
          loading={saveLoading}
        >
