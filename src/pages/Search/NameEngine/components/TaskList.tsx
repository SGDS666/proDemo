...
import { checkTaskIdSubmit, exTimes, exTimeToDateTime, getNamesFromString } from '@/utils/common';
...
const TaskList: React.FC<FormProps> = (props) => {
  ...
  const [state, setState] = useSetState<Record<string, any>>({
    ...
    userTreeData: [],
    owners: [],
    filterValues: { type: 'name' },
    scrollY: 600,
    sort: {},
    tblSelectKeys: [],
    showFilter: false,
    companyName: '',
    nameSearchType: 'single',
    nameCount: 0,
  ...
  const onNamesTextBlur = (value: string) => {
    setState({ nameCount: 0 });
    const names = getNamesFromString(value);
    setState({ nameCount: names.length });
  };
  ...
  const onSingleSearchAction = async () => {
    ...
    createRun({ name: companyName, type: 'name', language: 'EN', keyword: companyName });
    setState({ companyName: '' });
  };

  const onMultiSearchAction = async () => {
    const { nameWords } = state;
    const names = getNamesFromString(nameWords);
    if (!names.length) {
      message.error('未检出有效公司名称！');
      return;
    }
    let name = '';
    if (names.length === 1) {
  ...
  const renderSearchArea = () => {
    const { nameSearchType } = state;
    if (nameSearchType === 'single') {
      return (
        <Input.Search
          placeholder="输入公司名称关键词，挖掘相关邮箱信息、企业信息、联系人信息"
          style={{ width: '100%' }}
          size="large"
    ...
    return (
      <div>
        <Input.TextArea
          placeholder="请输入公司名称文本内容，自动识别，每行一个"
          rows={6}
          onBlur={(e) => onNamesTextBlur(e.target.value)}
          style={{ width: '100%', marginBottom: 12 }}
          value={state.nameWords}
          onChange={(e) => setState({ nameWords: e.target.value })}
        />
          ...
          <div style={{ width: 800 }}>
            ...
            >
              <Radio.Group
                buttonStyle="solid"
                size="large"
                value={state.nameSearchType}
                onChange={(e) => setState({ nameSearchType: e.target.value })}
              >
                <Radio.Button value="single">单个名称搜索</Radio.Button>
                <Radio.Button value="multi">批量搜索名称</Radio.Button>
              </Radio.Group>
            </Space>
            {renderSearchArea()}
          </div>
