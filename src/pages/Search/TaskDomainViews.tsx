...
import TaskDomains from './components/TaskDomains';
...
const viewType = 'searchDomain';
const TaskViews: React.FC<FormProps> = (props) => {
  ...
  useEffect(() => {
    pinViewsRun({ type: 'searchDomain' });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  return state.showAllViews ? (
    <ViewsList onClose={() => setState({ showAllViews: false })} viewType="searchDomain" />
  ) : (
    <Card title={false}>
      <Tabs
      ...
      <ViewRename
        ...
        actionReload={() => pinViewsRun({ type: 'searchDomain' })}
      />
    </Card>
  );
};
