      ...
      return s
        .replace(
          ...
          function (matcher, p1, p2, p3, offset, string) {
            return Buffer.from([parseInt(p1, 16), parseInt(p2, 16), parseInt(p3, 16)]).toString(
              'utf8',
            );
          },
        )
    ...
    try {
      ...
      function _append(headers, content) {
        ...
        if (encoding == 'base64') {
          ...
          // eslint-disable-next-line no-param-reassign
          // content = emlformat.unquotePrintable(content, charset); // 乱码问题
          // eslint-disable-next-line no-param-reassign
          content = iconv.decode(Buffer.from(quotedPrintable.decode(content), 'binary'), charset);
        } else if (
          charset != 'utf8' &&
        ...
        if (!result.html && contentType && contentType.toLowerCase().indexOf('text/html') >= 0) {
          ...
          !result.text &&
          contentType &&
          contentType.toLowerCase().indexOf('text/plain') >= 0
        ) {
          if (typeof content != 'string') {
            //content = content.toString("utf8");
            // eslint-disable-next-line no-param-reassign
